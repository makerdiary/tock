...
ci-travis:
	...
	@printf "$$(tput bold)*************$$(tput sgr0)\n"
	@for f in `./tools/list_tools.sh`; do echo "$$(tput bold)Build & Test $$f"; pushd tools/$$f > /dev/null; CI=true RUSTFLAGS="-D warnings" cargo build --all-targets || exit 1; popd > /dev/null; done
	@printf "$$(tput bold)*****************$$(tput sgr0)\n"
